...
Download the latest stable version of Prometheus from https://prometheus.io/download/#prometheus (avoid any pre-release version). As of this date, the latest stable release version is 2.33.5 . Adjust the following instructions accordingly if there is a newer stable release version with a different archive name. The file name should end with *linux-amd64.tar.gz* (for linux and AMD64 instructions set).
...
```console
$ sha256sum prometheus-2.33.5.linux-amd64.tar.gz
```
...
```console
$ rm -rf prometheus-2.33.5.linux-amd64
$ rm prometheus-2.33.5.linux-amd64.tar.gz
```
